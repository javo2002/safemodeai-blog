...
interface SavedPostData extends Omit<PostData, "sources"> {
  sources?: string[] // Actual stored format
}

interface PostEditorProps {
  initialData?: SavedPostData
  onSave: (data: SavedPostData) => void
}
...
export function PostEditor({ initialData, onSave }: PostEditorProps) {
  ...
  const handleSubmit = (publishedStatus: boolean) => {
    if (!formData.title.trim() || !formData.category) {
      // Content can be empty for "Coming soon"
      toast({
    ...
    const postToSave: SavedPostData = {
      ...formData,
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              <Label htmlFor="content" className="text-[#EAEAEA] mb-2 block">
                Content (leave blank for "Coming soon...")
              </Label>
              <Textarea
                id="content"
